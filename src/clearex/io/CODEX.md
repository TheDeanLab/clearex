...
- Root store attr `spatial_calibration` is the canonical store-level
  stage-to-world mapping for Navigate multiposition placement metadata.
- Missing `spatial_calibration` attrs resolve to identity
  (`z=+z,y=+y,x=+x`).
...
- Materialization and store initialization must preserve an existing
  ...
  stores without rewriting canonical `data`.
...
- For `experiment.yml` inputs, explicit overrides are written after store
  materialization.
- For existing Zarr/N5 stores, explicit overrides update the root attr before
  analysis starts.
...
- Headless CLI now supports `--channel-indices` for uSegment3D, including `--channel-indices all` to process the full channel axis.
- Headless CLI also supports `--input-resolution-level` for uSegment3D pyramid-level selection.
