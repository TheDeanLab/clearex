...
- Canonical metadata/provenance namespaces are:
  ...
  - `clearex/gui_state`
  - `clearex/results`
- Canonical-store reuse checks validate the public OME source collection with
  `ome-zarr-models` before deciding whether an existing `data_store.ome.zarr`
  can be reused.
...
- When `MicroscopeState.is_multiposition` is true, infer position count from
  `multi_positions.yml` before trusting `experiment.yml` counters; check both
  `Saving.save_directory` and the local `experiment.yml` directory so remote
  Windows save paths still resolve local sidecars.
- Materialization must canonicalize `multi_positions.yml` rows into lowercase
  stage-coordinate dictionaries under `clearex/metadata["stage_rows"]` and
  derive `clearex/metadata["position_translations_zyx_um"]` from the persisted
  `spatial_calibration` mapping.
...
- Repairing copied or relocated multiposition stores must be metadata-only:
  update the source experiment pointers, `stage_rows`, and derived position
  translations, but do not rewrite canonical source arrays or analysis outputs.
