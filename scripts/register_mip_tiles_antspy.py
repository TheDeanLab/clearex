    ...
    for channel in channels:
        ...
        for tile_index, position in enumerate(positions):
            ...
            if transform is None:
                warped = moving_canvas
                warped_mask = moving_mask_canvas
            else:
                moving_img = ants.from_numpy(moving_canvas)
                moving_mask_img = ants.from_numpy(moving_mask_canvas.astype(np.float32))
            ...
            sum_image += warped * warped_mask
            weight_image += warped_mask
